...
class CallSiteValidator(private val context: IrPluginContext) {
    ...
    /**
     * Phase 3.7: Validate demand hints from dependency modules.
     * Demand hints are generated by A2 in non-@KoinApplication modules for types
     * that couldn't be resolved locally. The @KoinApplication module validates
     * them against its full assembled graph.
     */
    fun validateDemandHintsFromDependencies(
        assembledGraphTypes: Set<String>,
        dslDefinitions: List<Definition>,
        annotationProcessor: KoinAnnotationProcessor,
        dslHintGenerator: DslHintGenerator
        ...
        for (hintFuncSymbol in hintFunctions) {
            ...
            // Skip framework whitelist and @Provided types
            if (BindingRegistry.isWhitelistedType(targetFqName)) continue
            if (ProvidedTypeRegistry.isProvided(targetFqName)) continue

            if (targetFqName in allKnownTypes) {
                KoinPluginLogger.debug { "  OK: $targetFqName -- found in graph" }
                continue
            }

            // Extract file info for error location
            val hintFilePath = try {
                (hintFunc.parent as? IrFile)?.fileEntry?.name
            } catch (_: Exception) { null }

            KoinPluginLogger.error(
                "Missing definition: $targetFqName\n" +
                "  Required by a dependency module (demand hint from A2 validation).\n" +
                "  No matching definition found in any declared module.\n" +
                "  Check your module includes in @KoinApplication.",
                hintFilePath, -1, -1
            )
            errorCount++
        }
        ...
        if (errorCount > 0) {
            KoinPluginLogger.debug { "Phase 3.7: $errorCount unresolved demand hints" }
        } else {
            KoinPluginLogger.debug { "Phase 3.7: All demand hints satisfied" }
        }
    }
